...
import com.playground.payment.fixture.presentation.request.PaymentMethodRequestTestFixture
...
class PaymentMethodRegisterApiTest(
    ...
    @Test
    @WithMockAuthUser(userId = 5L)
    fun `결제 수단 등록 성공`() {
        val request = PaymentMethodRequestTestFixture.registerRequest()
        val mockPaymentMethod = PaymentMethodDomainTestFixture.mockPaymentMethod(
            id = 11L,
            userId = 5L,
    ...
    @Test
    @WithMockAuthUser
    fun `결제 수단 등록 실패 - 필수 항목이 비어있음`() {
        val invalidRequest = PaymentMethodRequestTestFixture.registerRequest(
            authKey = "",
            cardCompany = "",
            cardNumberMasked = "",
        )
        ...
        performAndDocument("결제 수단 등록 실패 - 필수 항목이 비어있음") {
            ...
            additionalMatchers = arrayOf(
                ...
                jsonPath("$.errors.authKey").value(notNullValue()),
                jsonPath("$.errors.cardCompany").value(notNullValue()),
                jsonPath("$.errors.cardNumberMasked").value(notNullValue()),
            )
            snippets = arrayOf(
                responseFields(
